...
; Q(a): With the same-type-coercion installed, what happens if apply-generic is
;       called with two argemunts of the same type for an operation that is not
;       found in the table for that type? (e.g. (exp z1 z2))
; A: We no longer get an error saying that there is no method for these types.
;   Instead, the program runs into an infinite loop. (Does not crash because of
;   tail calls. Ha!)
;
; Q(b): Is Louis correct that something had to be done about coercion with
;       arguments of the same type, or does apply-generic work correctly as is?
; A: Since the apply-generic only tries coercion if it does not have a procedure
;    for the particular types it should be fine.
;    However, there should probably be something done about the fact that the
;    procedure hangs if useless coercions happen to be defined.
