...
'(
  ...
  after-call16
    (assign argl (op list) (reg val))
    (restore env)
    ; we restore env but do not save it again for evaluating the last argument
    (save argl)
    (assign proc (op lookup-variable-value) (const *) (reg env))
    (assign val (op lookup-variable-value) (const product) (reg env))
    (assign argl (op list) (reg val))
  ...
  after-call13
    ...
    (restore proc)
    (restore continue)
    ; we do not save env/continue before entering the recursion.
    ; thus, no additional stack spaced is used.
    (test (op primitive-procedure?) (reg proc))
