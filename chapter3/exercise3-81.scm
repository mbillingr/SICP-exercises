...
(define (controlled-rand control-stream)
  (define (control number ctrl)
    (if (eqv? ctrl '?)
        (rand-update number)
        ctrl))
  (define numbers
    (cons-stream
      (control 42 (stream-car control-stream))
      (stream-map control numbers (stream-cdr control-stream))))
  numbers)
